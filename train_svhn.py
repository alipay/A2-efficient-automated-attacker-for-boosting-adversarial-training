...
def get_args():
    ...
    parser.add_argument('--attack', default='auto', type=str, choices=['pgd', 'none', 'auto'])
...
def main():
    ...
    logger_test = PlainLogger(os.path.join('./', f'svhn-{args.model}-{args.attack_iters}-{epsilon}.txt'))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (X, y) in tqdm(enumerate(train_loader), desc=f"Epoch {epoch}:\t"):
            if args.eval:
                break
            X, y = X.cuda(), y.cuda()
            if args.mixup:
