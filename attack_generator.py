...
def eval_robust(model, test_loader, perturb_steps, epsilon, step_size, loss_fn, category, rand_init, desc):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.enable_grad():
        for data, target in tqdm(test_loader, total=len(test_loader), desc=desc):
            data, target = data.cuda(), target.cuda()
