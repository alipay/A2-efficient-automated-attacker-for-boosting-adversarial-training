...
from attack_generator import eval_robust
...
if __name__ == '__main__':
    ...
    if 'fgsm' in args.attack:
        loss, robust_acc = eval_robust(model, test_loader, perturb_steps=1, epsilon=0.031, step_size=0.031, loss_fn="cent", category="Madry", rand_init=True, desc='FGSM')
        print(f'FGSM Test Accuracy: {100 * robust_acc :.2f}%')
    if 'pgd20' in args.attack:
        loss, robust_acc = eval_robust(model, test_loader, perturb_steps=20, epsilon=0.031, step_size=0.031 / 4, loss_fn="cent", category="Madry", rand_init=True, desc="PGD20")
        print(f'PGD20 Test Accuracy: {100 * robust_acc :.2f}%')
    if 'cw' in args.attack:
        loss, robust_acc = eval_robust(model, test_loader, perturb_steps=30, epsilon=0.031, step_size=0.031 / 4, loss_fn="cw", category="Madry", rand_init=True, desc="CW")
        print(f'CW Test Accuracy: {100 * robust_acc :.2f}%')

    if 'aa' in  args.attack:
